...
char getHTTPResult(Client client) {
  while (client.read() != '4') {} 
...
void queryServer(String ping)
{
	...
	if (ping=="") {
	    client.print("GET http://192.168.1.31/cgi-bin/getLockCommand");
	} else {
	    client.print("GET http://192.168.1.31/updateLockStatus?ping=");
	    client.print(ping);
        }
...
int findTag( char tagValue[10] ) {
  for (int thisCard = 0; thisCard < numberOfTags; thisCard++) {
    ...
    //Serial.println( allowedTags[thisCard]);
...
void loop() {
  ...
  if (rfid.available()) {
	...
	if((val == 0x0D)||(val == 0x0A)||(val == 0x03)||(val == 0x02)||(val == 0xFF)) {
	    //Serial.print(val, HEX);
	    //Serial.println("Junk");
	    if (((val == 0x02) || (val==0xFF)) && (bytesRead==0)) {
	...
	if( tagId > 0 )
	{
	    String name=tagName[tagId - 1];   // Get the name for this tag from the database
	    Serial.print("Authorized tag ID ");
	    queryServer(name);					   // Data is sent every 5 seconds
	    Serial.print(tagId);
	    Serial.print(": unlocking for ");
	    Serial.println(name);   // Get the name for this tag from the database
    ...
    Serial.println("Input not available");
    queryServer("");
    delay(20);
    }
}
